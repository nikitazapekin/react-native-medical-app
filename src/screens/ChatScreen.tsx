...
interface Message {
  ...
  avatar: any;
  sender?: string;
  isServerMessage?: boolean; // Флаг для сообщений с сервера
}
...
export default function ChatScreen({ route }: UserEditChildrenProps) {
  ...
  const loadChatHistory = async (userData: UserData) => {
    if (!chatId || !userData.role) return;
    ...
    try {
      ...
      const transformedMessages: Message[] = history.map(msg => ({
        ...
        avatar: Avatar,
        sender: msg.from,
        isServerMessage: true // Помечаем как сообщение с сервера
      }));

      console.log('Loaded chat history, messages count:', transformedMessages.length);
      setMessages(transformedMessages);
    } catch (error: any) {
      console.error('Error loading chat history:', error);
      if (error.message === 'SESSION_EXPIRED') {
  ...
  const handleIncomingMessage = (messageData: any) => {
    ...
    // Проверяем, нет ли уже такого сообщения (по ID или по содержанию + времени)
    const isDuplicate = messages.some(msg => 
      msg.id === messageData.id || 
      (msg.text === messageData.message && 
       Math.abs(new Date(msg.time).getTime() - new Date(messageData.time).getTime()) < 1000)
    ...
    // Убираем ID из временных если это подтверждение нашего сообщения
    if (tempMessageIds.size > 0) {
      setTempMessageIds(new Set());
    }
  ...
  const handleSendMessage = (text: string) => {
    ...
    const tempMessage: Message = {
      ...
      avatar: Avatar,
      sender: senderType,
      isServerMessage: false // Помечаем как временное сообщение
    };
